...
function checkNodeVersion() {
  ...
  if (majorVersion < 22) {
    ...
    log(`📋 Required: Node.js v22.0.0 or higher`, 'yellow');
...
function promptForProjectName(rl) {
  return new Promise((resolve) => {
    ...
    log('   • Press Enter to use default name "my-app"', 'yellow');
    console.log('');
...
${colors.blue}📋 Next steps:${colors.reset}
  ...
  ${colors.yellow}npm run dev${colors.reset}        # Run with tsx for fast dev
  
  ${colors.yellow}npm run build${
    colors.reset
  }      # Transpile typescipt to javascipt
  ${colors.yellow}npm run start${colors.reset}      # Run build
